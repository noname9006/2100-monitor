...
class AnalyzeQuestComplete {
    constructor() {
        ...
        // Current date info - parse from provided UTC string
        this.currentDate = new Date('2025-10-01T12:22:04.000Z');
        ...
        console.log('🎯 AnalyzeQuest Complete - Enhanced Parallel Processing');
        console.log(`📅 Current Date: ${this.currentDate.toISOString()}`);
        console.log(`👤 User: noname9006`);
        console.log(`🎨 Mode: Parallel analysis with fixed future date visibility and no data extension`);
    ...
    async fetchAllMintingHistoryParallel() {
        console.log('\n📡 Fetching complete minting history (parallel with enhanced error handling)...');
        ...
        const contractPromises = this.contracts.map(async (contract) => {
            ...
            try {
                            ...
                            for (const event of events) {
                                try {
                                    const block = await this.retryRpcCall(
                                        ...
                                        2 // Even fewer retries for individual blocks
                                    );
                                    ...
                                    // Continue processing other events instead of failing the whole chunk
                                }
                            }
                ...
                return { contract: contract.name, events: totalEventsFound, failedChunks };
                
            } catch (error) {
                console.error(`   ❌ [${contract.name}] Error fetching history: ${error.message}`);
                return { contract: contract.name, events: 0, error: error.message };
            }
        });
        ...
        results.forEach(result => {
            if (result.error) {
                ...
                console.log(`   ✅ ${result.contract}: ${result.events.toLocaleString()} events`);
    ...
    processAllData() {
        ...
        for (const contract of this.contracts) {
            ...
            console.log(`   ✅ Processed ${processedData.length} active days`);
    ...
    async generateCumulativeStackedChart() {
        ...
        for (let i = 0; i < contractsWithMints.length; i++) {
            ...
            ctx.font = '9px Arial';
            ctx.fillStyle = '#666666';
            const stats = `${totalMints.toLocaleString()} mints • ${activeDays} days`;
        ...
        console.log(`   🏷️  Legend: ${legendColumns} columns, ${legendRows} rows (only active contracts)`);
    ...
    /**
     * Generate combined daily mints chart with grey bars, moving average line, and constrained smooth colorful task lines
     */
    async generateCombinedDailyMintsChart() {
        console.log('\n🎨 Generating combined daily mints chart with enhanced end date visibility...');
        ...
        // Calculate max values across all data
        let maxDailyMints = 0;
        ...
        console.log(`   📈 Chart max (rounded to 100): ${maxRounded.toLocaleString()}`);
        
        // Title
        ctx.fillStyle = '#333333';
        ctx.font = 'bold 28px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Daily Tasks Completion', canvasWidth / 2, 35);
        ...
        // Draw combined daily mints as grey bars (50% opacity)
        ...
        // Draw 1-week moving average line (75% opacity grey) - constrained smooth curve
        ...
        // Draw constrained smooth colorful task lines for individual daily mints (90% opacity, 1.5x wider)
        for (const contract of this.contracts) {
            const taskPoints = [];
            
            // Collect ALL points including zeros to maintain curve accuracy
        ...
        for (let i = 0; i < this.contracts.length; i++) {
            ...
            ctx.font = '9px Arial';
            ctx.fillStyle = '#666666';
            const stats = `${totalMints.toLocaleString()} total • ${activeDays} days`;
            ctx.fillText(stats, x + 20, y + 12);
    ...
    getSummary() {
        const summary = {
            ...
            processingMode: 'Enhanced Parallel with Retry Logic',
            chartTypes: ['Cumulative stacked area (no future data, enhanced end date visibility)', 'Combined daily mints with enhanced end date visibility (90% opacity)'],
...
// Main execution
async function main() {
    console.log('🎯 AnalyzeQuest Complete - Enhanced End Date Visibility & No Future Data Extension');
    console.log('📋 Configuration: PNG Export Only, Enhanced Parallel Processing');
    console.log('🎨 Output: 2 PNG charts (Cumulative Stacked + Combined Daily with Enhanced End Date Visibility)');
    ...
    console.log('📊 Charts: Enhanced end date visibility (Oct 6), no future data extension');
    console.log('🎨 Features: End date always visible, blank future space, constrained curves');
    ...
    console.log('\n🎉 Enhanced End Date Visibility Analysis Complete!');
    ...
    console.log('📊 Chart 1 fixes:');
    console.log('   • Enhanced end date visibility: Oct 6 guaranteed to be visible on X-axis');
    console.log('   • No future data extension: Blank space for days that haven\'t started yet');
    console.log('   • Zero-mint filtering: Contracts with 0 mints completely excluded');
    console.log('   • Proper data boundaries: Areas stop at actual data, not extended to future');
    console.log('📊 Chart 2 fixes:');
    console.log('   • Enhanced end date visibility: Oct 6 guaranteed to be visible on X-axis');
    console.log('   • Grey bars (50% opacity) showing combined daily mints from all tasks');
    console.log('   • Standard width bars for clean appearance');
    console.log('   • Constrained smooth grey moving average line (75% opacity) with 1-week window');
    console.log('   • Constrained smooth colorful task lines (90% opacity, 1.5x wider) that cannot go below zero');
    console.log('   • Y-axis rounded to 100 intervals for cleaner scale');
    console.log('   • getWeeklyMarkersWithEndDate: Ensures end date is always included in X-axis markers');
}
